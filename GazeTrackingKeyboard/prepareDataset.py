...
parser.add_argument(
    '--dataset_path', help="Path to extracted files. It should have folders called '%%05d' in it.")
...
def main():
    ...
    recordings = recordings[[os.path.isdir(
        os.path.join(args.dataset_path, r)) for r in recordings]]
    recordings.sort()
    ...
    pbar = tqdm(recordings)
    for i, recording in enumerate(pbar):
        pbar.set_description(f'Processing Recording {recording}')
        ...
        # Preprocess
        allValid = np.logical_and(appleFace['IsValid'], faceGrid['IsValid'])
        if not np.any(allValid):
            continue

        frames = np.array(
            [int(re.match('(\d{5})\.jpg$', x).group(1)) for x in frames])

        def bboxFromJson(data): return np.stack(
            (data['X'], data['Y'], data['W'], data['H']), axis=1).astype(int)
        # for compatibility with matlab code
        faceBbox = bboxFromJson(appleFace) + [-1, -1, 1, 1]
        faceGridBbox = bboxFromJson(faceGrid)

        fbar = tqdm(frames)
        for j, frame in enumerate(fbar):
            fbar.set_description(f'Processing Frames {frame}')
            # Can we use it?
            if not allValid[j]:
                continue
    ...
    meta['labelFaceGrid'] = np.stack(
        meta['labelFaceGrid'], axis=0).astype(np.uint8)
    ...
    # Find mapping
    mKey = np.array(['%05d_%05d' % (rec, frame) for rec, frame in zip(
        meta['labelRecNum'], meta['frameIndex'])], np.object_)
    rKey = np.array(['%05d_%05d' % (rec, frame) for rec, frame in zip(
        reference['labelRecNum'], reference['frameIndex'])], np.object_)
    ...
    meta['labelVal'] = np.ones(
        (len(meta['labelRecNum'],)), np.bool_)  # default choice
    ...
    print('======================\n\tSummary\n======================')
    print('Total added %d frames from %d recordings.' %
          (len(meta['frameIndex']), len(np.unique(meta['labelRecNum']))))
    if nMissing > 0:
...
def cropImage(img, bbox):
    ...
    res[aDst[1]:bDst[1], aDst[0]:bDst[0],
        :] = img[aSrc[1]:bSrc[1], aSrc[0]:bSrc[0], :]
